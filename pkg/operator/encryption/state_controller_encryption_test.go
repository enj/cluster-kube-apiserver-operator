...
func TestEncryptionStateController(t *testing.T) {
	scenarios := []struct {
		...
		validateFunc func(ts *testing.T, actions []clientgotesting.Action, targetNamespace string, expectedEncryptionCfg *apiserverconfigv1.EncryptionConfiguration)
	}{
		...
		// it also checks the content and the order of encryption providers, this test expects identity first and aescbc second
		{
			...
			// TODO: figure why base64 of 61def964fb967f5d7c44a2af8dab6865 doesn't match
			//       the test expects NjFkZWY5NjRmYjk2N2Y1ZDdjNDRhMmFmOGRhYjY4NjUK whereas the actual value is NjFkZWY5NjRmYjk2N2Y1ZDdjNDRhMmFmOGRhYjY4NjU=
			//expectedEncryptionCfg: createEncryptionCfgNoWriteKey("1", "NjFkZWY5NjRmYjk2N2Y1ZDdjNDRhMmFmOGRhYjY4NjUK", "secrets"),
			expectedEncryptionCfg: createEncryptionCfgNoWriteKey("1", "NjFkZWY5NjRmYjk2N2Y1ZDdjNDRhMmFmOGRhYjY4NjU=", "secrets"),

			validateFunc: func(ts *testing.T, actions []clientgotesting.Action, destName string, expectedEncryptionCfg *apiserverconfigv1.EncryptionConfiguration) {
				if len(actions) != 5 {
					ts.Fatalf("expected to get 5 actions but got %d", len(actions))
				}
